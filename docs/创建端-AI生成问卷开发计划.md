...
> 状态：**已完成（全部开发点含联调验收均已交付，2026-09-02）**
...
| 1-2 profile 接口扩展（返回打码 aiConfig + aiConfigured） | ✅ 已完成 | 2026-09-01 | 未配置时不返回 aiConfig 字段；用户不存在按 401 处理 |
| 1-3 前端：UserInfo 昵称下拉菜单 + AISettingsModal | ✅ 已完成 | 2026-09-01 | |
...
| 开发点 | 状态 | 完成时间 | 备注 |
|---|---|---|---|
| 2-1 后端：zod componentSchema + AiService（chatWithRetry/normalizeOptions）+ POST /api/ai/generate-question | ✅ 已完成 | 2026-09-01 | |
...
| 开发点 | 状态 | 完成时间 | 备注 |
|---|---|---|---|
| 3-1 前端：services/ai.ts + ManageLayout 侧边栏按钮 + AIGenerateModal（未配置引导） | ✅ 已完成 | 2026-09-01 | |
...
| 开发点 | 状态 | 完成时间 | 备注 |
|---|---|---|---|
| 4-1 联调验收 | ✅ 已完成 | 2026-09-02 | 用户真实 Key 验收通过；功能点 2/3 计划均将其列为已完成前置依赖 |
