...
> 状态：**方案设计已完成（取舍点已按推荐方案确认，2026-09-03），可进入开发**
...
| 访谈结束判定 | **轮次上限 + 前端「结束访谈」按钮**双保险 | 不追求 AI 精确判定结束（自然语言判定脆弱）；提示词约束「提纲问完输出收尾语」，前端可随时手动结束 |
...
| 开发点 | 内容 | 依赖 |
|---|---|---|
| 6-1 | 后端 Question 数据模型扩展 | 无 |
| 6-2 | 后端 Answer 数据模型扩展 | 无 |
| 6-3 | 创建端：新建访谈入口 + 独立配置页（手动提纲） | 6-1 |
| 6-4 | 创建端：AI 生成提纲 | 6-3 |
| 6-5 | 后端：SSE 流式访谈接口 | 6-1 |
| 6-6 | 填写端：聊天 UI + 流式接收 | 6-5 |
| 6-7 | 填写端：结束访谈 + 提交答卷 | 6-2、6-6 |
| 6-8 | 后端：访谈 AI 总结 + 访谈答卷列表 | 6-2 |
| 6-9 | 统计端：访谈统计视图 | 6-8 |
